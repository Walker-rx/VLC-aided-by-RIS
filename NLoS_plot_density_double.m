...
yd_for_plot_yz = 9;  zd_for_plot_yz = 9;
p_LoS_save = [];
p_NLoS_save = [];
    ...
    for j = 1:length(yd_save)
        ...
        for ii = 1:k
            for jj = 4:l
                ...
                angle_RIS_yz = findReflect_angle(coor_led,coor_PD,coor_RIS_yz);
                ...
                if all((angle_RIS_yz > 0) & (angle_RIS_yz < pi))
                    ...
                    if all(abs(coor_reflect_yz-coor_RIS_yz)<1e-7)
                        ...
                        diff_tmp_yz = NaN;
                        fprintf("NaN happen in yz wall, i=%d,j=%d,ii=%d,jj=%d",i,j,ii,jj);
                        eval(strcat(str_diff_yz+'('+string(ii)+','+string(jj)+')'+'='+string(diff_tmp_yz)+';'));
                        eval(strcat(str_y_unuse_yz,'=[',str_y_unuse_yz,','+string(yr+wm/2+(jj-1)*(wm+delta_wm))+'];'));
                        eval(strcat(str_z_unuse_yz,'=[',str_z_unuse_yz,','+string(zr+hm/2+(ii-1)*(hm+delta_hm))+'];'));
                    end                    
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure
eval(strcat('X = RIS_x_use'+string(xd_for_plot)+'_'+string(zd_for_plot),';'));
eval(strcat('Z = RIS_z_use'+string(xd_for_plot)+'_'+string(zd_for_plot),';'));
plot(X,Z,'ro', 'MarkerFaceColor', 'red', 'MarkerSize', 8)
hold on
eval(strcat('X = RIS_x_unuse'+string(xd_for_plot)+'_'+string(zd_for_plot),';'));
eval(strcat('Z = RIS_z_unuse'+string(xd_for_plot)+'_'+string(zd_for_plot),';'));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure
eval(strcat('Y = RIS_y_use_yz_'+string(yd_for_plot_yz)+'_'+string(zd_for_plot_yz),';'));
eval(strcat('Z = RIS_z_use_yz_'+string(yd_for_plot_yz)+'_'+string(zd_for_plot_yz),';'));
plot(Y,Z,'ro', 'MarkerFaceColor', 'red', 'MarkerSize', 8)
hold on
eval(strcat('Y = RIS_y_unuse_yz_'+string(yd_for_plot_yz)+'_'+string(zd_for_plot_yz),';'));
eval(strcat('Z = RIS_z_unuse_yz_'+string(yd_for_plot_yz)+'_'+string(zd_for_plot_yz),';'));
plot(Y,Z,'bo', 'MarkerFaceColor', 'blue', 'MarkerSize', 8)
xlim([0,8])
ylim([1,3])
xlabel('Y');
ylabel('Z');
