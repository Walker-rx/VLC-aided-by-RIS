function [] = NLoS_singlePD_plot_polar(pd_azi)
    phi_semi = 60/180*pi;
    xi_fov = 85/180*pi;
    f = 1.5;
    T = 1.0;
    ...
    pd_azi_save = pd_azi; 
    pd_polar_min = pi/2; pd_polar_max = pi;
    pd_polar_save = pd_polar_min : (pd_polar_max-pd_polar_min)/100 : pd_polar_max;
    ...
    for i = 1:length(pd_azi_save)
        pd_azi_loop = pd_azi_save(i);
        for j = 1:length(pd_polar_save)
            pd_polar = pd_polar_save(j);
            angle_PD = [pd_polar,pd_azi_loop];
    ...
    figure
    polarplot(pd_polar_save-pi/2,p_LoS_save,'b+-')
    hold on
    polarplot(pd_polar_save-pi/2,p_NLoS_save+p_LoS_save,'ro-','MarkerFaceColor','r')
    legend( '无IRS辅助','存在IRS辅助','Location', 'northeastoutside')
    title(sprintf('方位角为%.1f度时，PD端功率与极角关系',pd_azi_loop*180/pi))
    % set(gca,'FontSize',15);

    % figure
    % polarplot(pd_polar_save-pi/2,rate_LoS_save,'b+-')
    % hold on
    % polarplot(pd_polar_save-pi/2,rate_total_save,'ro-','MarkerFaceColor','r')
    % legend( '无IRS辅助','存在IRS辅助','Location', 'northeastoutside')
    % title(sprintf('方位角为%.1f度时，PD端速率与极角关系',pd_azi_loop*180/pi))
    % set(gca,'FontSize',15);
end
