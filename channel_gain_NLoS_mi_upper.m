function h_gain_NLoS = channel_gain_NLoS_mi_upper(source_point,D_point,reflect_point,source_angle,D_angle,RIS_angle,phi_semi,A_RIS,f,xi_fov,T,rho_mi)
%%%%%%%% f: internal refractive index       T: gain of the optical filter    
    ...
    RIS_normal = [sin(RIS_angle(1))*cos(RIS_angle(2)) sin(RIS_angle(1))*sin(RIS_angle(2)) cos(RIS_angle(1))];
    ...
    theta_RS = acos(dot(RS_vect,RIS_normal)/(norm(RS_vect)*norm(RIS_normal)));

    G = f^2/(sin(xi_fov)^2);
    A_eff = A_RIS *G *T *cos(theta_RS);
    if theta_RD > xi_fov
        h_gain_NLoS = 0;
    else
        h_gain_NLoS = rho_mi*(m+1)*cos(phi_RS)^(m)*cos(theta_RD)*cos(theta_RS)*A_eff/(2*pi*(RS_norm2)^2);
    end    
end